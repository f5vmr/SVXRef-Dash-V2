...
if (isProcessRunning('svxreflector')) {

echo "<table style=\"margin-top:4px;margin-bottom:13px;\">\n";
echo "<tr><th><span style=\"font-size:12px;\">Active Logics</span></th></tr>\n";
  if ((defined('REFCONFIG')) && (defined('SVXCONFPATH'))) {
    $REFCONFIGFile = SVXCONFPATH . "/" . REFCONFIG;}
else {$REFCONFIGFile = SVXCONFPATH."/".REFCONFIG;
  }
 ...
 // $inReflectorDefaultLang = explode(",", $REFCONFIG['ReflectorLogic']['DEFAULT_LANG']);
...
if (($check_logics[0]=="RepeaterLogic") && (isset($REFCONFIG['RepeaterLogic']['MODULES'])))
{ $modules = explode(",",str_replace('Module','',$REFCONFIG['RepeaterLogic']['MODULES'])); }
if (($check_logics[0]=="SimplexLogic") && (isset($REFCONFIG['SimplexLogic']['MODULES'])))
{ $modules = explode(",",str_replace('Module','',$REFCONFIG['SimplexLogic']['MODULES'])); }
else
$modules=""; 
$modecho = "False";
$inReflectorDefaultLang = explode(",", $REFCONFIG['ReflectorLogic']['DEFAULT_LANG']);
...
$tgdefault = $REFCONFIG['ReflectorLogic']['DEFAULT_TG'];
$tgmon = explode(",",$REFCONFIG['ReflectorLogic']['MONITOR_TGS']);
...
//echo "</table>\n";
if (($check_logics[0]=="RepeaterLogic") && ($REFCONFIG['RepeaterLogic']['TX'] !== "NONE")) {
  echo "<table  style=\"margin-bottom:13px;\"><tr><th>Repeater Status</th></tr><tr>";
  echo getTXInfo();
  echo "</table>\n"; }
if (($check_logics[0]=="SimplexLogic") && ($REFCONFIG['SimplexLogic']['TX'] !== "NONE")) {
  echo "<table  style=\"margin-bottom:13px;\"><tr><th>Radio Status</th></tr><tr>";
  echo getTXInfo();
  echo "</table>\n"; }
